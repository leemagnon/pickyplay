...
const UpdateUserInfo = () => {
  ...
  // 클래스 명
  const EmailInput = emailValidationError ? 'error' : null;
  const UserEmailAuthCodeInput = userEmailAuthCodeRequiredError || userEmailAuthCodeValidationError ? 'error' : null;
  const NicknameInput = nicknameValidationError || duplicatedNicknameCheckRequiredError ? 'error' : null;
  const PasswordCheckInput = passwordCheckRequiredError || passwordCheckError ? 'error' : null;
  const PasswordInput = passwordLengthError ? 'error' : null;
  ...
  const onSubmit = useCallback(() => {
    if (email !== '') {
      if (userEmailAuthCode === '') { // 사용자가 인증 코드를 입력하지 않음
        setUserEmailAuthCodeValidationError(false);
      ...
      if (userEmailAuthCode !== '' && emailAuthCode !== userEmailAuthCode) {
        setUserEmailAuthCodeRequiredError(false);
    ...
    console.log(email, userEmailAuthCode, password, passwordCheck, nickname, profileImg);

    dispatch({
      type: UPDATE_USER_INFO_REQUEST,
      data: { email, password, nickname, profileImg },
    });
  }, [email,
    emailAuthCode,
    userEmailAuthCode,
