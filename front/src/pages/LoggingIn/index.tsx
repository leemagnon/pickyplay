    ...
    <Background>
      ...
      <LogInBody>
        <form onSubmit={onSubmit}>
          ...
          <Input type="email" className={IsEmailError ? 'error' : ''} id="email" name="email" value={Email} onChange={onChangeEmail} placeholder="이메일 주소" />
          {IsEmailError && <InputError className="font-nanum-gothic">정확한 이메일 주소를 입력하세요.</InputError>}
          <Input type="password" className={IsPasswordError ? 'error' : ''} id="password" name="password" value={Password} onChange={onChangePassword} placeholder="비밀번호" />
