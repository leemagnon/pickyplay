...
const DetailedMovieModal = ({ data, onCloseModal }) => {
  ...
  return (
    <Background className="scroll" browserWidth={browserSize.browserWidth} browserHeight={browserSize.browserHeight}>
      <CloseModalButton onClick={onCloseModal} browserWidth={browserSize.browserWidth}>&times;</CloseModalButton>
      {
        data.stlls[0] === "" ? 
          <Title>{data.title}</Title> 
          : 
          <Slider {...settings} style={{width: '100%', overflow: 'hidden', marginBottom: '30px'}}>
      ...
      <Modal visible={showMoreActorModal}>
        <div style={{textAlign: 'center', padding: '30px 40px', background: 'white'}}>
          <CloseMoreActorModalButton onClick={() => setShowMoreActorModal(false)}>&times;</CloseMoreActorModalButton>
          {data.actors.map((v,i) => {
